function [PVector,Feature] = FuncGetInitial4(PVector,Feature,Image,PoseID,K)
...
for i=1:nRowNum-1;
    ...
    if FID > nRowNumF;
        ...
        V1 = Funcuv2V(Feature(FID,7:8),PVector.Pose(6*(PoseID-1)+1:6*(PoseID-1)+3,1)',K);
        [PVector.Feature(2*FID-1,1),PVector.Feature(2*FID,1)] = FuncV2PT(V1);
        clear V1;
        continue;
    ...
    elseif Feature(FID,5) == 1;
        ...
        V2 = Funcuv2V(Feature(FID,10:11),PVector.Pose(6*(PoseID-1)+1:6*(PoseID-1)+3,1)',K);
        PVector.Omega(FID,1) = FuncV2O(V1,V2);
        clear V1 V2;
        continue;
    
    elseif Feature(FID,5) >= 2;                
%         if PVector.Omega(FID,1)<0.02;           
%             V2 = Funcuv2V(Image(i+1,2:3),PVector.Pose(6*(PoseID-1)+1:6*(PoseID-1)+3,1)',K);
...
%             end;
%             [Max,ID] = max(Omegai);
%             if Max>PVector.Omega(FID,1)*1.2 || Max>0.02;
%                 PVector.Omega(FID,1) = Max;
