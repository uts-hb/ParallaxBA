function [PVector,Feature] = malaga_FuncGetInitial3_02(PVector,Feature,Image,PoseID,K,input_value,GT_P0)
if strcmp(input_value, 'Estimated')
    if PoseID == 1;
        ...
        %     PVector.Pose(6*(PoseID-1)+1:6*(PoseID-1)+3,1) = Image(1,1:3)';
        %     PVector.Pose(6*(PoseID-1)+4:6*PoseID,1) = Image(1,4:6)';
    end
end 
if  strcmp(input_value, 'Ground_truth')
    %     Ground Truth input
    if PoseID == 1;
        PVector.Pose(6*(PoseID-1)+1:6*PoseID,1) = GT_P0(1,1:6)';
    else
        PVector.Pose(6*(PoseID-1)+1:6*(PoseID-1)+3,1) = GT_P0(PoseID,1:3)';
        PVector.Pose(6*(PoseID-1)+4:6*PoseID,1) = GT_P0(PoseID,4:6)';
    end
end
